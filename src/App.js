...
const App = () => {
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:5000/followers").then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    await axios.post("http://127.0.0.1:5000/add", { date, count: followers });
    alert("Data added!");
    setFollowers("");
    setDate("");
  ...
  const handleEditSubmit = async (e) => {
    ...
    await axios.put("http://127.0.0.1:5000/update", { date: editDate, count: followers });
    alert("Data updated!");
    setFollowers("");
    setEditDate("");
    setEditMode(false);
  ...
  const handleForecast = async (days) => {
    try {
      const response = await axios.get(`http://127.0.0.1:5000/forecast?days=${days}`);
